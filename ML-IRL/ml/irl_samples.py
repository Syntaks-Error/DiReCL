...
def try_evaluate(itr: int, policy_type: str, sac_info):
    ...
    metrics = eval.KL_summary(
        expert_samples, agent_emp_states.reshape(-1, agent_emp_states.shape[2]), env_steps, policy_type
    )
    # eval real reward
    real_return_det = eval.evaluate_real_return(
        sac_agent.get_action, env_fn(), v["irl"]["eval_episodes"], v["env"]["T"], True
    )
...
if __name__ == "__main__":
    ...
    for itr in range(v["irl"]["n_itrs"]):
        if v["sac"]["reinitialize"] or itr == 0:
            # Reset SAC agent with old policy and new environment
        ...
        samples = collect.collect_trajectories_policy_single(
            gym_env, sac_agent, n=v["irl"]["training_trajs"], state_indices=state_indices
        )
        ...
        for _ in range(v["reward"]["gradient_step"]):
            if v["irl"]["resample_episodes"] > v["irl"]["expert_episodes"]:
                expert_res_indices = np.random.choice(
                    expert_trajs.shape[0], v["irl"]["resample_episodes"], replace=True
                )
                expert_trajs_train = expert_trajs[expert_res_indices].copy()  # resampling the expert trajectories
            elif v["irl"]["resample_episodes"] > 0:
                expert_res_indices = np.random.choice(
                    expert_trajs.shape[0], v["irl"]["resample_episodes"], replace=False
                )
