...
def evaluate_real_return(policy, env, n_episodes, horizon, deterministic):
    ...
    for _ in range(n_episodes):
        ...
        for t in range(horizon):
            ...
            obs, rew, done, _ = _step_env(env, action)  # NOTE: assume rew=0 after done=True for evaluation
            ret += rew
            if done:
                break
        returns.append(ret)
...
def do_eval(v, reward_func, device):
    ...
    eval_sac_kwargs["learning_starts"] = gym_env.T * v["evaluation"]["random_explore_episodes"]
