#!/usr/bin/env bash
# Run a training of the RL model via Stable-Baselines3 PPO (PyTorch)
# example invocation
...
# Train
python -m commonroad_rl.train_model_sb3 --uuid none --algo ppo --seed "$SEED" --eval-freq -1 -f "$LOG_PATH" --n_envs "$NUM_CPUS" -n "$STEPS" \
...
# --debug
# -i ../log/ppo/commonroad-v1_4/best_model.zip
