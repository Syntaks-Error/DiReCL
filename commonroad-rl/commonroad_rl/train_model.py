...
def run_stable_baselines_argsparser():
    ...
    parser.add_argument(
        "-i", "--trained-agent", type=str, default="", help="Path to a pretrained agent to continue training"
    )
    parser.add_argument(
        "--algo", help="RL Algorithm", default="ppo2", type=str, required=False, choices=list(ALGOS.keys())
    )
    parser.add_argument("-n", "--n-timesteps", help="Set the number of timesteps", default=int(1e6), type=int)
    parser.add_argument("--log-interval", help="Override log interval (default: -1, no change)", default=-1, type=int)
    parser.add_argument(
        "--eval-freq", default=10000, type=int, help="Evaluate the agent every n steps (if negative, no evaluation)"
    )
    ...
    parser.add_argument(
        "--configs-path", type=str, default="", help="Path to file for overwriting environment configurations"
    )
    parser.add_argument(
        "--hyperparams-path", type=str, default="", help="Path to file for overwriting model hyperparameters"
    )
    parser.add_argument(
        "--optimize-observation-configs", action="store_true", default=False, help="Optimize observation configurations"
    )
    parser.add_argument(
        "--optimize-reward-configs", action="store_true", default=False, help="Optimize reward configurations"
    )
    parser.add_argument(
        "--guided", action="store_true", default=False, help="Guided optimization of some reward parameters"
    )
    parser.add_argument(
        "--optimize-hyperparams", action="store_true", default=False, help="Optimize model hyperparameters"
    )
    ...
    parser.add_argument(
        "--sampler", type=str, default="tpe", choices=["random", "tpe", "skopt"], help="Sampler for optimization"
    )
    parser.add_argument(
        "--pruner", help="Pruner for optimization", type=str, default="median", choices=["halving", "median", "none"]
    )
...
def continue_learning(hyperparams, args, env, tensorboard_log, callbacks, normalize, save_path, env_id, rank):
    ...
    model = ALGOS[args.algo].load(
        args.trained_agent, env=env, tensorboard_log=tensorboard_log, verbose=args.logging_mode.value, **hyperparams
    )
...
def optimize_parameters(
    ...
    if args.optimize_hyperparams:
        ...
        report_name = (
            f"report_{args.algo}_{args.env}-{args.n_trials}-"
    ...
    if args.optimize_reward_configs:
        ...
        report_name = (
            f"report_{args.algo}_{args.env}-{args.n_trials}-trials"
    ...
    if args.optimize_observation_configs:
        ...
        os.makedirs(log_path, exist_ok=True)
        optimized_observation = optimize_observation_configs(
            args.algo,
            args.env,
            create_model,
            create_env,
...
def train_from_scratch(hyperparams, args, env, tensorboard_log, callbacks, save_path, env_id, normalize, rank):
    ...
    if args.algo == "gail":
        ...
        model = ALGOS[args.algo](
            env=env, expert_dataset=dataset, tensorboard_log=tensorboard_log, seed=args.seed, verbose=1, **hyperparams
        )
    else:
        model = ALGOS[args.algo](
            env=env, tensorboard_log=tensorboard_log, seed=args.seed, verbose=args.logging_mode.value, **hyperparams
        )
...
def run_stable_baselines(args):
    ...
    if args.save_freq > 0:
        ...
        callbacks.append(
            CheckpointCallback(
                save_freq=args.save_freq, save_path=save_path, name_prefix="rl_model", verbose=args.logging_mode.value
            )
        )
        callbacks.append(
            SaveVecNormalizeCallback(
    ...
    def create_env(n_envs, eval_env=False, **kwargs):
        ...
        ### add gail model
        if args.algo == "gail":
            env1 = gym.make("commonroad-v1", **env_configs)
