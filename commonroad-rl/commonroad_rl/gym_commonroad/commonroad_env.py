...
class CommonroadEnv(gym.Env):
    ...
    def reset(
        ...
        *,
        ...
        if options is not None:
            benchmark_id = options.get("benchmark_id", benchmark_id)
            scenario = options.get("scenario", scenario)
            planning_problem = options.get("planning_problem", planning_problem)
    ...
    def step(self, action: Union[np.ndarray, State]):
        """
        ...
        truncated = bool(termination_info.get("is_time_out", False))
        if truncated:
            terminated = False
    ...
    def reset_renderer(
        self, renderer: Union[MPRenderer, None] = None, draw_params: Union[MPDrawParams, dict, None] = None
    ) -> None:
        if renderer:
    ...
    def render(self, mode: str = "human", **kwargs) -> None:
        ...
        ego_obstacle = DynamicObstacle(
            ...
            obstacle_shape=Rectangle(
                length=self.ego_action.vehicle.parameters.l, width=self.ego_action.vehicle.parameters.w
            ),
        ...
        if not self.render_configs["render_combine_frames"] or self.terminated:
            if isinstance(mode, int):
                filename = (
                    os.path.join(self.visualization_path, str(self.scenario.scenario_id), self.file_name_format % mode)
                    + ".png"
                )
            else:
                filename = (
    ...
    def _set_scenario_problem(
        self, benchmark_id=None, scenario: Scenario = None, planning_problem: PlanningProblem = None
    ) -> None:
        """
        ...
        if scenario is None or planning_problem is None:
            if len(self.all_problem_dict) == 0:
                mode_name = "test" if getattr(self, "is_test_env", False) else "train"
                raise ValueError(
                    f"No scenarios available in {mode_name} reset config. "
                    "Please generate/populate pickle reset files before calling reset()."
                )
    ...
    def get_ttc_lead_follow(observation_dict):
        ...
        ttc_follow = get_ttc(
            observation_dict["lane_based_p_rel"][idx_follow], observation_dict["lane_based_v_rel"][idx_follow]
        )
        ttc_lead = get_ttc(
            observation_dict["lane_based_p_rel"][idx_lead], observation_dict["lane_based_v_rel"][idx_lead]
        )
