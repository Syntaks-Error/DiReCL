...
class SurroundingObservation(Observation):
    ...
    def __init__(self, configs: Dict, config_name: str = "surrounding_configs"):
        ...
        self.dummy_dist_intersection: float = configs.get("dummy_dist_intersection", 50.0)
        ...
        if self.observe_lane_circ_surrounding:
            ...
            self.max_obs_dist = np.sqrt(
                (self.lane_rect_sensor_range_length / 2) ** 2 + (self.lane_rect_sensor_range_width / 2) ** 2
            )
        ...
        assert (
            sum(
                [
                    self.observe_lidar_circle_surrounding,
                    self.observe_lane_rect_surrounding,
                    self.observe_lane_circ_surrounding,
                ]
            )
            <= 1
        ), "Only one kind of surrounding observation can be active!"
    ...
    def build_observation_space(self) -> OrderedDict:
        ...
        if self.observe_lane_rect_surrounding or self.observe_lane_circ_surrounding:
            ...
            observation_space_dict["lane_based_p_rel"] = gym.spaces.Box(
                -self.max_obs_dist, self.max_obs_dist, (6,), dtype=np.float32
            )
            if self.observe_vehicle_type:
                observation_space_dict["vehicle_type"] = gym.spaces.Box(
                    0, self.num_vehicle_types - 1, (6,), dtype=np.int8
                )
        ...
        # Lidar-based elliptical surrounding observation
            ...
            observation_space_dict["lidar_circle_dist_rate"] = gym.spaces.Box(
                -np.inf, np.inf, (num_beams,), dtype=np.float32
            )
            observation_space_dict["lidar_circle_dist"] = gym.spaces.Box(
                -self.max_obs_dist, self.max_obs_dist, (num_beams,), dtype=np.float32
            )

            if self.observe_vehicle_type:
                observation_space_dict["vehicle_type"] = gym.spaces.Box(
                    0, self.num_vehicle_types - 1, (num_beams,), dtype=np.int8
                )
            if self.observe_vehicle_lights:
                observation_space_dict["vehicle_signals"] = gym.spaces.Box(-1, 1, (num_beams,), dtype=np.int8)

            if self.reward_safe_distance_coef != 0:
                observation_space_dict["dist_lead_follow_rel"] = gym.spaces.Box(
                    -self.max_obs_dist, self.max_obs_dist, (2,), dtype=np.float32
                )
        ...
        if self.observe_intersection_distances:
            observation_space_dict["intersection_distances"] = gym.spaces.Box(
                -self.dummy_dist_intersection, self.dummy_dist_intersection, (6,), dtype=np.float32
            )
        if self.observe_ego_distance_intersection:
            observation_space_dict["ego_distance_intersection"] = gym.spaces.Box(
    ...
    def observe(
        ...
        ego_vehicle: Vehicle,
        time_step: int,
        connected_lanelet_dict: Union[None, Dict] = None,
        ...
        if not self.fast_distance_calculation:
            self._ego_shape = Rectangle(
                length=ego_vehicle.parameters.l,
                width=ego_vehicle.parameters.w,
                center=self._ego_state.position,
                orientation=self._ego_state.orientation,
            ).shapely_object
        self._collision_checker = collision_checker
        self._local_ccosy = local_ccosy
        self.lanelet_dict, self.all_lanelets_set = SurroundingObservation.get_nearby_lanelet_id(
            connected_lanelet_dict, ego_lanelet
        )
        ...
        if self.observe_lane_rect_surrounding or self.observe_lane_circ_surrounding:
            ...
            if self.observe_lane_rect_surrounding:
                ...
                self._surrounding_area = pycrcc.Circle(
                    self.lane_circ_sensor_range_radius, self._ego_state.position[0], self._ego_state.position[1]
                )
            ...
            self._surrounding_area = pycrcc.Circle(
                self.lidar_sensor_radius, self._ego_state.position[0], self._ego_state.position[1]
            )
        ...
        if self.observe_intersection_velocities or self.observe_intersection_distances:
            if self.observe_lane_circ_surrounding:
                intersection_observation = conflict_zone.generate_intersection_observation(
                ...
                print(
                    "ERROR:SurroundingObservation: Intersection observations currently only selectable together "
    ...
    def draw(self, render_configs: Dict, render: MPRenderer, terminated: bool = False):
        ...
        if render_configs["render_surrounding_area"] and (
            ...
            self._surrounding_area.draw(
                render, draw_params={"facecolor": "lightblue", "edgecolor": "lightblue", "opacity": 0.5}
            )
                ...
                if obs is not None:
                    render.dynamic_artists.append(
                        LineDataUnits(
                            ...
                            color=color,
                            marker="*",
                            zorder=ZOrders.OBSTACLES + 1,
                            label="surrounding_obstacles_lane_based",
                        )
                    )
            ...
            for idx, detection_point in enumerate(self._detection_points):
                render.dynamic_artists.append(
                    LineDataUnits(
                        ...
                        color="b",
                        marker="1",
                        zorder=ZOrders.INDICATOR_ADD,
                        label="surrounding_obstacles_lidar_based",
                    )
                )
    ...
    def _get_surrounding_obstacles_lane_based(
        self, surrounding_area: Union[pycrcc.RectOBB, pycrcc.Circle]
        ...
        obstacle_lanelet, adj_obstacle_states, adj_obstacles = self._filter_obstacles_in_adj_lanelet(
            lanelet_ids, obstacle_states, obstacles, self.all_lanelets_set
        )
        rel_vel, rel_pos, detected_states, detected_obstacles, ego_vehicle_lat_position = self._get_rel_v_p_lane_based(
            obstacle_lanelet, adj_obstacle_states, self.lanelet_dict, adj_obstacles
        )
    ...
    def _add_relative_priority(
        ...
        if self._local_ccosy.cartesian_point_inside_projection_domain(
            ...
            ego_curv_coords = self._local_ccosy.convert_to_curvilinear_coords(
                self._ego_state.position[0], self._ego_state.position[1]
            )
            ego_proj_pos = self._local_ccosy.convert_to_cartesian_coords(ego_curv_coords[0], 0)
            t_type = self.trajectory_type_from_path(
                self._local_ccosy.reference_path(),
                turn_threshold,
                pycrcc.Circle(scan_range, ego_proj_pos[0], ego_proj_pos[1]),
            )
        ...
        for obs in obstacles:
            ...
            if len(obs_occupied_lanelet_id) > 1:
                obs_occupied_lanelet_id = Navigator.sorted_lanelet_ids(
                    obs_occupied_lanelet_id, obstacle_state.orientation, obstacle_state.position, self._scenario
                )[0]
            ...
            if ego_lanelet.lanelet_id in intersection_dict and (
                (
                    obs_lanelet.lanelet_id in intersection_dict
                    and intersection_dict[ego_lanelet.lanelet_id] == intersection_dict[obs_lanelet.lanelet_id]
                )
                ...
                # The signs match -> check right before left
                    ...
                    ego_lanelet_priority in [4, 5, 6]
                    and obstacle_lanelet_priority in [4, 5, 6]
                    or self._matching_signs(ego_lanelet, obs_lanelet)
                ):
    ...
    def _matching_signs(self, ego_lanelet: Lanelet, obs_lanelet: Lanelet):
        sign_ids_obs = list(
            self._scenario.lanelet_network.find_traffic_sign_by_id(s).traffic_sign_elements[0].traffic_sign_element_id
            for s in obs_lanelet.traffic_signs
        )
        sign_ids_ego = list(
            self._scenario.lanelet_network.find_traffic_sign_by_id(s).traffic_sign_elements[0].traffic_sign_element_id
            for s in ego_lanelet.traffic_signs
        )
        return any(
            s_id == s_id_ego
            and s_id
            ...
            for s_id in sign_ids_obs
            for s_id_ego in sign_ids_ego
        )
    ...
    def _detect_lanelet_priority(self, lanelet: Lanelet, traj_type: TrajectoryType) -> int:
        ...
        sign_ids = list(
            self._scenario.lanelet_network.find_traffic_sign_by_id(s).traffic_sign_elements[0].traffic_sign_element_id
            for s in lanelet.traffic_signs
        )
    ...
    def _get_surrounding_obstacles_lidar_circle(self) -> List[Obstacle]:
        ...
        obstacle_distances, observed_obstacles = self._get_obstacles_with_surrounding_beams(
            obstacle_shapes, detected_obstacles, surrounding_beams_ego_vehicle
        )

        distances, distance_rates, detection_points = self._get_distances_lidar_based(
            surrounding_beams_ego_vehicle, obstacle_distances
        )
    ...
    def _get_obstacle_shapes_in_surrounding_area(
        self, surrounding_area: pycrcc.Shape
    ...
    def _get_obstacles_with_surrounding_beams(
        self,
    ...
    def _get_distances_lidar_based(
        self, beams: List[Tuple[np.ndarray, float, float]], obstacle_distances: Union[List[float], np.ndarray]
        ...
        detection_points = [
            ...
            for i, ((beam_start, _, beam_angle), closest_collision) in enumerate(zip(beams, obstacle_distances))
        ]

        return dists, dist_rates, detection_points

    def _get_obstacles_in_surrounding_area(
        self, surrounding_area: pycrcc.Shape
            ...
            if o.prediction is not None:
                ...
                if (
                    ...
                    if surrounding_area.collide(obstacle_point):
                        ...
                        o_lanelet_id = self._get_occupied_lanelet_id(
                            self._scenario, o_center_lanelet_ids, obstacle_state
                        )
                        if o_lanelet_id is None:
                            # use shape lanelet assignment instead
                            o_lanelet_id = self._get_occupied_lanelet_id(
                                self._scenario, o_shape_lanelet_ids, obstacle_state
                            )
                            if o_lanelet_id is None:
                                ...
                                if len(non_empty_id_sets) == 0:
                                    continue
                                o_lanelet_id = self._get_occupied_lanelet_id(
                                    self._scenario, list(non_empty_id_sets[-1]), obstacle_state
                                )
    ...
    @staticmethod
    def _get_occupied_lanelet_id(
        scenario: Scenario, obstacle_lanelet_ids: List[int], obstacle_state: State
    ) -> Union[None, int]:
        """
        ...
        if len(obstacle_lanelet_ids) > 1:
            ...
            return Navigator.sorted_lanelet_ids(
                obstacle_lanelet_ids, obstacle_state.orientation, obstacle_state.position, scenario
            )[0]
    ...
    @staticmethod
    def _filter_obstacles_in_adj_lanelet(
        lanelet_ids: List[int], states: List[State], obstacles: List[Obstacle], all_lanelets_set: Set[int]
    ) -> Tuple[List[int], List[State], List[Obstacle]]:
        """
    ...
    @staticmethod
    def get_rel_v_p_follow_leading(
        distance_sign: int,
        distance_abs: float,
        p_rel_follow: float,
        p_rel_lead: float,
    ...
    def _get_rel_v_p_lane_based(
        ...
        adj_obstacles: List[Obstacle],
    ) -> Tuple[List[float], List[float], List[State], List[Obstacle], np.array]:
        """
        ...
        v_rel_left_follow, v_rel_same_follow, v_rel_right_follow, v_rel_left_lead, v_rel_same_lead, v_rel_right_lead = [
            0.0
        ] * 6
        ...
        try:
            ...
            (
                obstacle_left_follow,
                obstacle_left_lead,
                obstacle_right_follow,
                obstacle_right_lead,
                obstacle_same_follow,
                obstacle_same_lead,
            ) = [None] * 6

            for o_state, o_lanelet_id, obstacle in zip(obstacle_states, obstacles_lanelet_ids, adj_obstacles):

                distance_abs, distance_sign = self._get_ego_obstacle_distance(
                    obstacle, o_state, (ego_vehicle_long_position, ego_vehicle_lat_position)
                )
    ...
    def _get_ego_obstacle_distance(
        self, obstacle: Obstacle, obstacle_state: State, ego_curvi: Tuple[float, float]
    ) -> Tuple[float, int]:
        """
        ...
        try:
            o_curvi_long_position, _ = self._local_ccosy.convert_to_curvilinear_coords(
