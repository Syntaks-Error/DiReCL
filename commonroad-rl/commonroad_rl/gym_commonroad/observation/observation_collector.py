...
class ObservationCollector:
    ...
    def __init__(self, configs: Dict):
        ...
        self._enable_conflit_zone = (
            self.surrounding_observation.observe_intersection_velocities
    ...
    def observe(self, ego_vehicle: Vehicle) -> Union[np.array, Dict]:
        ...
        observation_dict_goal = self.goal_observation.observe(
            self._ego_state,
            goal=self._goal_region,
    ...
    def _update_ego_lanelet_and_local_ccosy(self):
        ...
        self._ego_lanelet_ids = self.sorted_lanelets_by_state(
            self._scenario, self._ego_state, lanelet_polygons, lanelet_polygons_sg
        )
    ...
    def render(self, render_configs: Dict, render: MPRenderer):
        ...
        if render_configs["render_local_ccosy"]:
            ...
            line = LineDataUnits(
                reference_path_local_ccosy[:, 0],
                reference_path_local_ccosy[:, 1],
    ...
    def _get_lanelet_polygons(self, meta_scenario_id) -> Tuple[List[Tuple[int, Polygon]], pycrcc.ShapeGroup]:
        ...
        if lanelet_polygons is None:
            lanelet_polygons = [
                (lanelet.lanelet_id, lanelet.convert_to_polygon())
                for lanelet in self._scenario.lanelet_network.lanelets
            ]
