...
def block_until_training(rl_filepath, log_status=False, iter_num=-1, response_id=-1, success_markers=None):
    # Ensure that the RL training has started before moving on
    if success_markers is None:
        success_markers = ["fps step:"]

    while True:
        rl_log = file_to_string(rl_filepath)
        training_started = any(marker in rl_log for marker in success_markers)
        if training_started or "Traceback" in rl_log:
            if log_status and training_started:
